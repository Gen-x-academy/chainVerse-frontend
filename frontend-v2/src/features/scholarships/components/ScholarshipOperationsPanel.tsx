...
import { DEFAULT_ADMIN_CONFIG, previewAdminConfigChange, redactAdminConfig, validateAdminConfig, type ScholarshipAdminConfig } from '../admin-config';
...
export function ScholarshipOperationsPanel() {
  const [config, setConfig] = useState<ScholarshipAdminConfig>(DEFAULT_ADMIN_CONFIG);
  ...
  const migration = useMemo(
    () => runLegacyMigration(sampleLegacyRecords, { scholarshipId: 'scholarship-standard', dryRun: true }),
    []
  );
  ...
  const mappedExample = mapLegacyAidRecord(sampleLegacyRecords[0], 'scholarship-standard');
    ...
    <section className="mx-auto w-full max-w-6xl space-y-6 px-4 py-8 text-slate-900">
      <header className="space-y-2">
        ...
        <p className="max-w-2xl text-sm text-slate-600 md:text-base">
          Durable domain events, on-chain settlement reconciliation, resumable legacy migration, and
      ...
      >
        <p role="status" aria-live="polite">
          {events.length} unique event{events.length === 1 ? '' : 's'} after deduplication.
        </p>
        <ul className="list-disc pl-5" aria-label="Recent scholarship domain events">
          {events.map((event) => (
      ...
      >
        <ul className="space-y-2" aria-label="Settlement reconciliation results">
          {settlement.map((item) => (
            <li key={item.intentId} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
              <span className="font-medium">{item.intentId}</span> — {item.reconciledStatus}
              {item.mismatches.length > 0 && (
                <span className="text-amber-700"> ({item.mismatches.join(', ')})</span>
              )}
            </li>
          ))}
        </ul>
      ...
      >
        ...
        <p className="break-all font-mono text-xs text-slate-500">
          webhook secret ref: {redactAdminConfig(config).webhookSigningSecretRef}
        </p>
        <button
          type="button"
          onClick={() => setConfig((current) => ({ ...current, maxAwardAmount: current.maxAwardAmount + 500 }))}

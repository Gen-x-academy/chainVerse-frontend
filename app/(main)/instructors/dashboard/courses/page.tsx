  ...
  return (
    <div className="relative w-full max-w-7xl mx-auto px-2">
      <div className="flex items-center justify-between mt-4 mb-2">
        ...
        <Button onClick={() => router.push("/instructors/dashboard/courses/new")} variant="outline" className="ml-auto">+ Add Course</Button>
      </div>
      {/* Custom Tabs */}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center mt-10">
          ...
          <Button onClick={() => router.push("/instructors/dashboard/courses/new")}>Add Course</Button>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {courses.map(course => (
            <div
              key={course.id}
              className="rounded-2xl border border-gray-200 bg-white flex flex-col items-stretch shadow-sm overflow-hidden"
            >
              {/* Imagen centrada con fondo gris claro y padding */}
              ...
              <div className="flex gap-2 px-4 pb-4 bg-white">
                <button
                  ...
                  className="flex-1 py-2 rounded-md border border-gray-200 bg-gray-100 text-gray-900 font-medium hover:bg-gray-200 transition"
                >
                  Delete
                </button>
                <button
                  onClick={() => handleEdit(course.id)}
                  className="flex-1 py-2 rounded-md bg-[#4264FB] text-white font-medium hover:bg-blue-700 transition"
                >
                  Edit
                </button>
              </div>
